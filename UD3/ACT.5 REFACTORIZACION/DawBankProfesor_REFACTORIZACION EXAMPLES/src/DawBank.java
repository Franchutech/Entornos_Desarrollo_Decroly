...
public class DawBank {
    public static void main(String[] args) {
        ...
        clientePrincipal = tryMetodoPruebas(Nombre, DNI, fechaNac);
        if (clientePrincipal == null) return; // Si no hay clientePrincipal, no puedo seguir

        String IBAN = MiUtils.comprobarPatronRepetidamente(ibanPattern, "Introduce un IBAN");

        // 2. Creo la cuenta con el objeto Cliente
        CuentaBancaria miCuenta = new CuentaBancaria(IBAN, clientePrincipal);
        ...
        do {
            ...
            switch (opcion) {
                ...
                case "3":
                    // Accedo al nombre a través del objeto clientePrincipal
    ...
    private static Cliente tryMetodoPruebas(String Nombre, String DNI, LocalDate fechaNac) {
        Cliente clientePrincipal;
        try {
            // Cro el clientePrincipal (puede lanzar CuentaException por sus setters)
            clientePrincipal = new Cliente(Nombre, DNI, fechaNac, "600123123", "clientePrincipal@email.com", "Calle Mayor 1");
        } catch (CuentaException e) {
            System.out.println("Error al crear el clientePrincipal: " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    ...
    public void retirar(CuentaBancaria miCuenta) {
        Scanner sc = new Scanner(System.in);
